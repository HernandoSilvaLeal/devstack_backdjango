...
from apps.users.api.serializers import UserSerializer, UserListSerializer     # Importo el serializador para transformar la data de la BD a JSON
...
def user_api_view(request):                                                   # Logica REQUEST-GET, la envio, consulta db, serializa y responde json con HTTP code.
    ...
    if request.method == 'GET':
        users = User.objects.all().values('id', 'username', 'email', 'password', 'name') # Queryset
