...
from apps.standards.api.models import StandardsModel, ChaptersModel, ArticlesModel
from apps.standards.api.serializers import StandardsSerializer, ChaptersSerializer, ArticlesSerializer
class StandardsViewSet(viewsets.GenericViewSet):
    ...
    def get_measure_units(self, request):
        data = StandardsModel.objects.filter(state=True)
        data = StandardsSerializer(data, many=True)
...
class ChaptersViewSet(viewsets.GenericViewSet):
    model = ChaptersModel
    ...
    @action(detail=False, methods=['get'])
    def get_measure_units(self, request):
        data = ChaptersModel.objects.filter(state=True)
        data = ChaptersSerializer(data, many=True)
        return Response(data.data)
...
class ArticlesViewSet(viewsets.GenericViewSet):
    ...
    @action(detail=False, methods=['get'])
    def get_measure_units(self, request):
        data = ArticlesModel.objects.filter(state=True)
        data = ArticlesSerializer(data, many=True)
        return Response(data.data)
